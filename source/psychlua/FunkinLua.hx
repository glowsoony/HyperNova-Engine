...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String, ?forMidSong:Bool = false, ?canSkip:Bool = true, ?loop:Bool = false) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile)))
			{
				...
				game.videoCutscene = game.startVideo(videoFile, forMidSong, canSkip, loop);
				return true;
			}
			else
			{
        ...
        Lua_helper.add_callback(lua,"tweenShaderProperty", function(shaderName:String, prop:String, value:Dynamic, time:Float, easeStr:String = "linear", ?tag:String = 'shader') {
            ...
            if(shad != null)
            {
				...
				MusicBeatState.getVariables().set(tag, 
                    FlxTween.num(startVal, value, time, {
                    ease: ease,
