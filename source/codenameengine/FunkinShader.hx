...
class FunkinShader extends FlxShader implements IHScriptCustomBehaviour {
	...
	{
	  if (value == null) value = ShaderTemplates.defaultFragmentSource;
	...
	{
	  if (value == null) value = ShaderTemplates.defaultVertexSource;
	  glRawVertexSource = value;
  
	  var useBackCompat:Bool = true;
	  for (regex in ShaderTemplates.vertBackCompatVarList)
		if (!regex.match(value)) useBackCompat = false;
  
	  value = value.replace("#pragma header", useBackCompat ? ShaderTemplates.vertHeaderBackCompat : ShaderTemplates.vertHeader)
		.replace("#pragma body", useBackCompat ? ShaderTemplates.vertBodyBackCompat : ShaderTemplates.vertBody);
	...
	public function hget(name:String):Dynamic {
		...
		if (cl.startsWith("openfl.display.ShaderParameter")) return (field.__length > 1) ? field.value : field.value[0];
		else if (cl.startsWith("openfl.display.ShaderInput")) return field.input;
		return field;
	}
