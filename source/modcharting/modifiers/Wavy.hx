...
import modcharting.Modifier.ModifierMath;
...
/*
    ...
    -   Has 7 subValues:
        ...
        +   useAlt (changes it's math, if Wavy (uses sin) it will now use cos, if tanWavy (uses tangent) it will now use cotangent)
...
class Wavy extends Modifier
{
    ...
    function wavyMath(lane:Int)
    {
		...
		{
			if (usesAlt) returnValue = 260 * currentValue * FlxMath.fastCos(((Conductor.songPosition) * (speed) * 0.0008) + (lane / 4)) * 0.2;
			else returnValue = 260 * currentValue * FlxMath.fastSin(((Conductor.songPosition) * (speed) * 0.0008) + (lane / 4)) * 0.2;
		}
   	...
   	function tanWavyMath(lane:Int)
    {
		...
		{
			if (usesAlt) returnValue = 260 * currentValue * (1 / Math.sin(((Conductor.songPosition) * (speed) * 0.0008) + (lane / 4))) * 0.2;
			else returnValue = 260 * currentValue * Math.tan(((Conductor.songPosition) * (speed) * 0.0008) + (lane / 4)) * 0.2;
		}
