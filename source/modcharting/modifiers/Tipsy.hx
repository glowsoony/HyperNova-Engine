...
/*
    ...
    -   Added 4 subValues:
        ...
        +   UseAlt (changes it's math, if tipsy (uses sin) it will now use cos, if tanTipsy (uses tangent) it will now use cotangent).
...
class Tipsy extends Modifier //My idea is clever, make this more simple to use
{
    ...
    function tanTipsyMath(lane:Int, curPos:Float):Float
    {
        ...
        if (usesAlt) returnValue = currentValue * (1 / Math.sin((time + ((lane) % NoteMovement.keyCount) * subValues.get('period').value) * (5) * 1 * 0.2) * Note.swagWidth * 0.5);
        else returnValue = currentValue * (Math.sin((time + ((lane) % NoteMovement.keyCount) * subValues.get('period').value) * (5) * 1 * 0.2) * Note.swagWidth * 0.5);
    ...
    function tipsyMath(lane:Int, curPos:Float):Float
    {
        ...
        if (usesAlt) returnValue = currentValue * (FlxMath.fastCos((time + ((lane) % NoteMovement.keyCount) * subValues.get('period').value) * (5) * 1 * 0.2) * Note.swagWidth * 0.5);
        else returnValue = currentValue * (FlxMath.fastSin((time + ((lane) % NoteMovement.keyCount) * subValues.get('period').value) * (5) * 1 * 0.2) * Note.swagWidth * 0.5);
