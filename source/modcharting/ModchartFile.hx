...
class ModchartFile
{
    ...
    public function new(renderer:PlayfieldRenderer)
    {
        ...
        data = loadFromJson(Paths.formatToSongPath(PlayState.SONG.song), Difficulty.getString().toLowerCase() == null ? Difficulty.defaultList[PlayState.storyDifficulty] : Difficulty.getString().toLowerCase());
    ...
    public function loadFromJson(folder:String, difficulty:String):ModchartJson //load da shit
    {
        ...
        var moddyFile:String = Paths.json(folder + '/modchart-' + difficulty);
        var moddyFile2:String = Paths.json(folder + '/modchart');

        #if MODS_ALLOWED
        var moddyFileMods:String = Paths.modsJson(folder + '/modchart-' + difficulty);
