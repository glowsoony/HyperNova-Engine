...
class Huds extends FlxGroup
{
	...
	function create():Void
	{
		...
		if (!hudadded)
		{
			...
			timeTxt = new EditedFlxText(timeBarBG.x + (timeBarBG.width / 2) + (if (ClientPrefs.data.timeBarType == 'Song Name') 3 else 0), timeBarBG.y - 10,
				400, "", 32);
			...
			healthBar = new EditedFlxBar(hudStyle == 'HITMANS' ? healthBarBG.x + 50 : healthBarBG.x + 4,
				hudStyle == 'HITMANS' ? healthBarBG.y + 20 : healthBarBG.y + 4, RIGHT_TO_LEFT,
			...
			switch (edwhakVariable.contains(PlayState.instance != null ? PlayState.instance.boyfriend.curCharacter : (PlayState.SONG != null ? PlayState.SONG.player1 : 'bf')))
			{
				...
				case false:
					iconP1 = new EditedHealthIcon(PlayState.instance != null ? PlayState.instance.boyfriend.healthIcon : (PlayState.SONG != null ? PlayState.SONG.player1 : 'bf'),
						true);
				default:
					iconP1 = new EditedHealthIcon(PlayState.instance != null ? PlayState.instance.boyfriend.healthIcon : (PlayState.SONG != null ? PlayState.SONG.player1 : 'bf'),
						true); // if it crash for some reazon?
			}
			...
			iconP2 = new EditedHealthIcon(PlayState.instance != null ? PlayState.instance.dad.healthIcon : (PlayState.SONG != null ? PlayState.SONG.player2 : 'dad'),
				false);
			...
			scoreTxt.setFormat(hudStyle == 'HITMANS' ? Paths.font("DEADLY KILLERS.ttf") : Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER,
	...
	override public function update(elapsed:Float)
	{
		...
		if (hudadded)
		{
			...
			iconP1.x = (hudUsed == 'hitmans') ? (FlxG.width - 160) : healthBar.x
				+ (healthBar.width * (FlxMath.remapToRange(percent, 0, 100, 100, 0) * 0.01))
				+ (150 * iconP1.scale.x - 150) / 2
				- iconOffset;
			iconP2.x = (hudUsed == 'hitmans') ? 0 : healthBar.x
				+ (healthBar.width * (FlxMath.remapToRange(percent, 0, 100, 100, 0) * 0.01))
	...
	public function ratingsBumpScale()
	{
		...
		ratingsBumpTween = FlxTween.tween(ratings.scale, {x: 1.3 * ratingsScaleMultiplier, y: 1.3 * ratingsScaleMultiplier}, 0.1, {
			...
			onComplete: function(twn:FlxTween)
			{
				...
				{
					ratingsBumpTween2 = FlxTween.tween(ratings.scale, {x: 0, y: 0}, 0.1, {
						ease: FlxEase.circIn,
						onComplete: function(twn:FlxTween)
						{
		...
		noteRatingTween = FlxTween.tween(noteScore.scale, {x: 1, y: 1}, 0.1, {
			ease: FlxEase.circOut,
			onComplete: function(twn:FlxTween)
			{
	...
	public function ratingsBumpScaleOP()
	{
		...
		ratingsOPBumpTween = FlxTween.tween(ratingsOP.scale, {x: 1.3 * ratingsScaleMultiplier, y: 1.3 * ratingsScaleMultiplier}, 0.1, {
			ease: FlxEase.circOut,
			onComplete: function(twn:FlxTween)
			{
				...
				{
					ratingsOPBumpTween2 = FlxTween.tween(ratingsOP.scale, {x: 0, y: 0}, 0.1, {
						ease: FlxEase.circIn,
						onComplete: function(twn:FlxTween)
						{
		...
		noteRatingTweenOp = FlxTween.tween(noteScoreOp.scale, {x: 1, y: 1}, 0.1, {
			ease: FlxEase.circOut,
			onComplete: function(twn:FlxTween)
			{
