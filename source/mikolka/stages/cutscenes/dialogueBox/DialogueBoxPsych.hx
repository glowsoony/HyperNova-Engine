...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		FlxTween.tween(bgFade, {alpha: 0.5}, style.FADE_DURATION, {ease: FlxEase.linear});
						...
						if (i != lastCharacter)
						{
							switch (char.jsonFile.dialogue_pos)
							{
								case 'left':
									...
									if (char.x < char.startingPos + style.offsetXPos)
										char.x = char.startingPos + style.offsetXPos;
								case 'center':
									char.y += style.scrollSpeed * elapsed;
									if (char.y > char.startingPos + style.offsetYPos)
										char.y = char.startingPos + style.offsetYPos;
								case 'right':
									char.x += style.scrollSpeed * elapsed;
									if (char.x > char.startingPos - style.offsetXPos)
										char.x = char.startingPos - style.offsetXPos;
